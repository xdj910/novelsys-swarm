#!/usr/bin/env python3
"""
Final Orphan Component Detector v3.0
Fixes critical bug: recursive command scanning to include novel/ subdirectory

CRITICAL BUG FIX:
- Previous version only scanned .claude/commands/*.md
- Most commands are in .claude/commands/novel/*.md subdirectory
- This caused massive false positives for coordinators

Fixed in v3.0:
- Recursive scanning of all .md files in commands directory
- Comprehensive pattern recognition from v2.0
- Accurate execution chain analysis
"""
...
class FinalOrphanDetector:
    ...
    def scan_components(self):
        ...
        # CRITICAL FIX: Recursive scan of commands directory
    ...
    def generate_detailed_report(self, orphan_coordinators: List[str], orphan_agents: List[str]):
        ...
        report = f"""
FINAL ORPHAN COMPONENT ANALYSIS REPORT v3.0
Generated: {Path.cwd()}
Timestamp: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

=== BUG FIX VERIFICATION ===
Total Command Files Found: {len(self.commands)}
Commands in novel/ subdirectory: {commands_in_novel_dir}
Recursive Scan: ENABLED (fixed critical bug)
...
"""
        ...
        # Bug fix summary
        report += "=== CRITICAL BUG FIXED ===\n"
        report += "v2.0 Bug: Only scanned .claude/commands/*.md (missed novel/ subdirectory)\n"
        report += "v3.0 Fix: Recursive scanning of all commands with rglob('*.md')\n"
        report += f"Result: Found {commands_in_novel_dir} additional commands in novel/ subdirectory\n\n"
...
def main():
    print("Final Orphan Component Detector v3.0")
    print("CRITICAL BUG FIX: Recursive command scanning (includes novel/ subdirectory)")

    detector = FinalOrphanDetector()
    report = detector.run_analysis()

    # Save report
    timestamp = __import__('datetime').datetime.now().strftime('%Y%m%d_%H%M%S')
    report_path = f".claude/testing/final_orphan_analysis_{timestamp}.md"
    ...
    lines = report.split('\n')
    for line in lines:
        if 'Total Orphans:' in line or 'Orphan Rate:' in line or 'Commands in novel/' in line:
            print(f"  {line.strip()}")
